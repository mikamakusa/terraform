...
resource "aws_elasticache_cluster" "elasticache" {
  ...
  subnet_group_name            = element(aws_elasticache_subnet_group.ElasticacheSubnetGroup.*.name, lookup(var.elasticache[count.index], "subnet_group_id"))
  security_group_ids           = [element(var.SecGroup, lookup(var.elasticache[count.index], "security_group_ids"))]
...
resource "aws_elasticache_replication_group" "ElasticacheReplicationGroup" {
  ...
  automatic_failover_enabled    = lookup(var.RepGroup[count.index], "automatic_failover_enabled", true)
  ...
  transit_encryption_enabled    = lookup(var.RepGroup[count.index], "transit_encryption_enabled", true)
  auth_token                    = lookup(var.RepGroup[count.index], "auth_token")
  ...
  dynamic "cluster_mode" {
    for_each = lookup(var.RepGroup[count.index], "cluster_mode")
    content {
