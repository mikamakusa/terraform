resource "azurerm_container_registry_webhook" "registry_webhook" {
  ...
  resource_group_name = lookup(registry_webhook[count.index], "resource_group_id") == null ? var.resource_group_name : element(var.resource_group_name, lookup(registry_webhook[count.index], "resource_group_id"))
  location            = lookup(registry_webhook[count.index], "resource_group_id") == null ? var.resource_group_location : element(var.resource_group_location, lookup(registry_webhook[count.index], "resource_group_id"))
